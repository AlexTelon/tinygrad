...
class _CL:
  def post_init(self, device=None):
    ...
    self.cl_ctxs: List[cl.Context] = [cl.Context(devices=[x]) for x in platforms[getenv('CL_PLATFORM', 0).value] if x.name not in getenv('CL_EXCLUDE', "").value.split(",")] if device is None else [cl.Context(devices=[platforms[getenv('CL_PLATFORM', 0).value][device]])]
